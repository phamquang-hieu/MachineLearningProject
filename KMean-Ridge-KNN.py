...
def Kmeans_Ridge_KNN(X, y, train_index, test_index, p1, p2, alpha=0.01, n_clusters=2):
	...
	for i in range(len(idtrain)):
		y_train_pred[i]*=(1.00 + p1*df.loc[idtrain[i]]['CHAS']/100 + p2*df.loc[idtrain[i]]['RAD']/100)
		y_train_true[i]*=(1.00 + p1*df.loc[idtrain[i]]['CHAS']/100 + p2*df.loc[idtrain[i]]['RAD']/100)

	for i in range(len(idtest)):
		y_test_pred[i]*=(1.00 + p1*df.loc[idtest[i]]['CHAS']/100 + p2*df.loc[idtest[i]]['RAD']/100)
		y_test_true[i]*=(1.00 + p1*df.loc[idtest[i]]['CHAS']/100 + p2*df.loc[idtest[i]]['RAD']/100)
...
def kfold(p1, p2):
	...
	for train_index, test_index in kfold.split(X):
		r2_on_train, r2_on_test = Kmeans_Ridge_KNN(X, y, train_index, test_index, p1, p2)
...
if __name__ == '__main__':
	...
	print("Optimize p for hypothesis: p1 =", best_p1, "p2 =", best_p2, "r2_train:", best_r2_train, "r2_test:", best_r2_test)
